...
const account5 = {
  owner: 'ABHIJEET JHA',
  movements: [400, 10000, -700, -550, 900],
  interestRate: 2.7,
  pin: 5555,
};
...
const btnCreate = document.querySelector('.create_account');
...
btnCreate.addEventListener('click', function(e) {
  ...
  if(createUser && createPin && createInterest) {
    ...
    alert(`Account created successfully! Your username is ${newAccount.username}`);
  } else {
...
// });
// console.log('-----FOR-----');
// const finalBalance = movements.reduce(function (acc, mov) {
//   return acc + mov;
// }, 0);
...
//   console.log(`${value}: ${value}`); // map is not used
// });
...
// const check = data.filter(function (num) {
//   if(num >2) {
//     return num;
//   }
// });
...
// console.log(`Your maximum movement is ${max}`);
...
// const avgg = calcaverageHumanAge2([5, 2, 4, 1, 15, 8, 3]);
// console.log(`The average age is ${avgg}`);
...
//     .filter(age => age >= 18)
//     .reduce((acc, age, i, arr) => acc + age / arr.length, 0);
// const avggg = calcaverageHumanAge3([16, 6, 10, 5, 6, 1, 4]);
// console.log(`The average age is ${avggg}`);



// const calc = num => {
//   const sum = num.reduce((acc, cur) => acc + cur, 0);
//   const sub = num.reduce((acc, cur) => acc - cur, 0);
//   const mul = num.reduce((acc, cur) => acc * cur, 1);
//   const div = num.reduce((acc, cur) => acc / cur, 1);

//   return { sum, sub, mul, div };
// };

// const answer = calc([1, 2, 3, 4]);
// console.log(answer);
...
// const movL = movements.findLastIndex(mov => Math.abs(mov) > 1200);
// console.log(`Your largest movements was ${movements.length-movL} movements ago`);
...
btnLogin.addEventListener('click', function () {
  [...document.querySelectorAll('.movements__row')].forEach(function (row, i) {
    if (i % 2 === 0) {
      row.style.backgroundColor = '#7ae5f3ff';
    } else {
      row.style.backgroundColor = '#f1dfabff';
    }
  });
});
